...
export default function Footer() {
  ...
  return (
    <footer>
      <div className="relative isolate z-10 flex h-svh min-h-[626px] w-full items-end bg-background pt-20 text-foreground xl:pt-32">
        <div aria-hidden="true" className="absolute inset-0 z-0 overflow-hidden">
          <div className="dot-wave-bg absolute inset-0">
        ...
        <div ref={ref} className="container relative z-10 flex flex-col justify-between pb-8 xl:flex-row xl:pb-24">
          <div className="flex flex-col justify-start">
            <div className="relative w-fit">
              ...
              <div className="flex flex-wrap items-center md:flex-row-reverse md:gap-x-10 xl:gap-x-20">
                <h2
                  className={`w-full text-left font-ramillas text-[58px] text-primary leading-[1.2] md:w-fit md:text-[76px] xl:text-[120px] transition-all duration-1000 delay-200 ${
                    isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
                >
                  Potential
                ...
                >
                  <div className="h-auto w-fit">
                    <p className="w-[100px] text-center text-base md:text-lg xl:text-xl">WORK WITH US</p>
                  </div>
                  <span className="absolute top-0 left-0 size-[34px] rounded-tl border-white border-t-2 border-l-2" />
                  <span className="absolute right-0 bottom-0 size-[34px] rounded-br border-white border-r-2 border-b-2" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-14 flex w-full flex-col items-end space-y-2 self-end text-right font-lato text-base md:mt-8 md:text-lg xl:mt-0 xl:text-xl">
            <h2 className="font-bold font-helvetica text-[28px] leading-[1.1] md:text-[32px] xl:text-[40px]">Pixel Group</h2>
            <a target="_blank" href="https://maps.app.goo.gl/8MKndDLHhaKB6JjVA" rel="noopener noreferrer">
              <p className="!mt-10">Jl. Panjang Cidodol No. 83, Kebayoran Lama, Jakarta Selatan 12220</p>
            </a>
            <p className="flex items-center justify-end gap-2">
              <PhoneIcon /> Phone: +62-21-2942-8555
            </p>
            <p>
              <a target="_blank" href="https://wa.me/6281119220654?text=Hello" rel="noopener noreferrer" className="flex items-center justify-end gap-2">
                <WhatsAppIcon /> WhatsApp: +62 811-1922-0654
              </a>
            </p>
            <p>
              <a target="_blank" href="mailto:contact@pixelgroup.id" rel="noopener noreferrer" className="flex items-center justify-end gap-2">
                <EmailIcon /> Email: contact@pixelgroup.id
              </a>
            </p>
            <div className="!mt-10 flex justify-end gap-3">
              <a target="_blank" href="https://wa.me/6281119220654?text=Hello" rel="noopener noreferrer" aria-label="WhatsApp" className="grid size-12 place-content-center hover:text-primary transition-colors">
                <WhatsAppIcon size={24} />
              </a>
              <a target="_blank" href="https://www.linkedin.com/company/pixelgroupindonesia/" rel="noopener noreferrer" aria-label="LinkedIn" className="grid size-12 place-content-center hover:text-primary transition-colors">
                <LinkedInIcon />
              </a>
              <a target="_blank" href="https://www.instagram.com/pixelgroup.idn/" rel="noopener noreferrer" aria-label="Instagram" className="grid size-12 place-content-center hover:text-primary transition-colors">
                <InstagramIcon />
              </a>
            </div>
          </div>
...
function WhatsAppIcon({ size = 16 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" className={`inline-block shrink-0`} style={{ width: size, height: size }}>
      <path fillRule="evenodd" clipRule="evenodd" d="M8 1.333C4.042 1.333.833 4.542.833 8.5c0 .987.2 1.929.562 2.786l-.707 2.752a.5.5 0 00.613.613l2.752-.707A7.167 7.167 0 008 15.667c3.958 0 7.167-3.209 7.167-7.167S11.958 1.333 8 1.333z" fill="#F8FAFC" />
    </svg>
  );
}
...
function LinkedInIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="inline-block size-6">
      <path d="M6.239 3.876a2.31 2.31 0 11-4.619 0 2.31 2.31 0 014.619 0zM6.257 7.975H1.602V22.55h4.655V7.975zM13.688 7.975H9.06V22.55h4.628v-7.652c0-4.253 5.615-4.6 5.615 0v7.652h4.646V13.322c0-7.178-8.306-6.913-10.261-3.388V7.975z" fill="#F8FAFC" />
    </svg>
  );
}
